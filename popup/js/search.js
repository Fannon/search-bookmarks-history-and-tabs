...
export async function initExtension() {
  ...
  if (ext.opts.search.approach === "fuzzy") {
    ...
    throw new Error(
      `The option "search.approach" has an unsupported value: ${ext.opts.search.approach}`
    );
  }
  ...
  ext.dom.searchApproachToggle.addEventListener(
    "mouseup",
    toggleSearchApproach
  );
  if (ext.opts.general.debounce) {
    ext.dom.searchInput.addEventListener(
      "keyup",
      debounce(search, ext.opts.general.debounce)
    );
  } else {
  ...
  console.debug(
    "Init Performance: " + totalInitPerformance[0].duration + "ms",
    initPerformance
  );
...
async function search(event) {
  ...
  if (searchTerm) {
    if (searchMode === "tags") {
      ...
      throw new Error(
        `Unsupported option "search.approach" value: "${ext.opts.search.approach}"`
      );
    }
  ...
  ext.model.result = ext.model.result.filter(
    (el) => el.score >= ext.opts.score.minScore
  );
